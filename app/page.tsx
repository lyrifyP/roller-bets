...
export default function RollerBetsTracker() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (rawBets) {
        setBets(JSON.parse(rawBets) as Bet[]);
      } else {
  ...
  useEffect(() => {
    if (isClient) localStorage.setItem('rb.bets', JSON.stringify(bets));
  }, [bets, isClient]);
  ...
  const chartPoints = useMemo(() => cumulative.map((p, i) => ({ x: i, y: p.value, label: p.date })), [cumulative]);
  ...
  // Hydrate filters from URL when it changes
  useEffect(() => {
    if (!isClient) return;
  ...
  useEffect(() => {
    ...
    if (filter.status && filter.status !== 'All') params.set('status', filter.status); else params.delete('status');
      ...
      <div className={state.theme === 'dark' ? 'min-h-screen bg-slate-950 text-slate-100' : 'min-h-screen bg-slate-50 text-slate-900'}>
        {!isClient ? (
          ...
          <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
                ...
                <div className="flex-1 sm:flex-none inline-flex rounded-xl border border-slate-700 overflow-hidden">
                  {(['All', 'Pending', 'Won', 'Lost']).map((s) => (
                    <button
                      key={s}
                      type="button"
                      className={`flex-1 sm:flex-none px-2 sm:px-3 py-2 text-xs sm:text-sm ${filter.status === s ? 'bg-slate-700' : 'bg-slate-800/60 hover:bg-slate-700/60'}`}
                      onClick={() => setFilter(f => ({ ...f, status: s as BetStatus | 'All' }))}
                    >
                      {s}
                    </button>
                  ))}
            ...
            {toast && (
              <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-slate-900/90 text-slate-100 border border-slate-700 rounded-xl px-4 py-3 shadow-lg" aria-live="polite">
